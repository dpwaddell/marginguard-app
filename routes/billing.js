...
const { exchangeForOfflineToken } = require('../lib/shopify-client');
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      try {
        ...
        console.log('[Billing] Using fresh exchanged token for subscribe');
      } catch (exchErr) {
    ...
    const confirmationUrl = await createSubscription(req.shop, token);
