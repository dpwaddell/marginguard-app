...
function calculateOrderMargin({ order, variantCosts, costRules, settings }) {
  ...
  for (const rule of costRules) {
    ...
    const cost = resolveRuleCost(rule, netRevenue, 1, netRevenue);
...
function resolveRuleCost(rule, basisValue, quantity, orderNetRevenue) {
  let cost = 0;
  ...
  if (rule.cost_type === 'fixed') {
    ...
    const basis = rule.basis === 'line_value' ? basisValue : (orderNetRevenue !== undefined ? orderNetRevenue : basisValue);
