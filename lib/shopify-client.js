...
async function registerWebhooks(shop, accessToken) {
  ...
  let existingByTopic = {};
  try {
    const data = await graphqlRequest(shop, accessToken, LIST_WEBHOOKS_QUERY);
  ...
  for (const wh of desired) {
    ...
    try {
      if (existing) {
        const result = await graphqlRequest(shop, accessToken, UPDATE_WEBHOOK_MUTATION, {
          id: existing.id,
        ...
        const result = await graphqlRequest(shop, accessToken, CREATE_WEBHOOK_MUTATION, {
          topic: gqlTopic,
          webhookSubscription: { callbackUrl: wh.address },
        });
