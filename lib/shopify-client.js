...
async function registerWebhooks(shop, accessToken) {
  ...
  const desired = [
    ...
    { topic: 'customers/data_request', address: `${appUrl}/webhooks/customers/data_request` },
    { topic: 'customers/redact', address: `${appUrl}/webhooks/customers/redact` },
    { topic: 'shop/redact', address: `${appUrl}/webhooks/shop/redact` },
  ];
...
async function removeOrderTag(shop, accessToken, orderId, tag) {
  const gid = `gid://shopify/Order/${orderId}`;
  ...
  const result = await graphqlRequest(shop, accessToken, ORDER_UPDATE_MUTATION, {
    input: { id: gid, tags: existingTags.filter(t => t !== tag) },
  });
  const errors = result?.orderUpdate?.userErrors || [];
  if (errors.length) throw new Error(errors.map(e => e.message).join(', '));
}

module.exports = { graphqlRequest, registerWebhooks, addOrderTag, removeOrderTag, getValidToken, exchangeForOfflineToken };
