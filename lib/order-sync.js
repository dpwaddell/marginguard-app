...
const { graphqlRequest, removeOrderTag } = require('./shopify-client');
...
async function syncOrders(shop, accessToken) {
  ...
  while (hasNextPage) {
    ...
    for (const { node } of edges) {
      ...
      await processOrder(shop, order, costRules, settings, accessToken);
      processed++;
    }
...
async function processOrder(shop, order, costRules, settings, accessToken) {
  ...
  // Remove low-margin tag when order recalculates as healthy
  if (!result.low_margin && settings.order_tagging_enabled && accessToken) {
    const tag = settings.low_margin_tag || process.env.LOW_MARGIN_DEFAULT_TAG || 'marginguard_low_margin';
    removeOrderTag(shop, accessToken, orderId, tag).catch(err =>
      console.error(`[OrderSync] Failed to remove tag from ${order.name}:`, err.message)
    );
  }
...
async function recalculateShopMargins(shop, accessToken) {
  console.log(`[Recalculate] Starting for ${shop}`);
  const count = await syncOrders(shop, accessToken);
  console.log(`[Recalculate] Complete for ${shop}: ${count} orders updated`);
  return count;
}

module.exports = { syncOrders, processOrder, recalculateShopMargins };
