...
async function syncOrders(shop, accessToken) {
  ...
  const { data } = await client.get('/orders.json?status=any&limit=50&fields=id,name,processed_at,currency,line_items,total_discounts,discount_codes,financial_status');
